...
struct2geno(file = "RoumanicusMaxMiss63.recode.p.structure", TESS = FALSE, diploid = TRUE, FORMAT = 2,
            extra.row = 0, extra.col = 1, output = "roumanicus.geno")



obj.at = snmf("roumanicus.geno", K = 1:10, ploidy = 2, entropy = T, CPU = 1, project = "new")
...
coord.at = read.table("roum_coord.txt")

plot(coord.at, pch = 19,  xlab= "Longitude", ylab = "Latitude")
map(add = T, interior = F, col = "grey80")

grid=createGrid(10.0, 40.0, 34.0, 56.0, 1000, 1000)


lColorGradients = list( c("gray95",brewer.pal(9,"Purples")),
