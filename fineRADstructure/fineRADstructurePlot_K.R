...
mcmcfile<-"populations_chunks.mcmc.xml" ## finestructure mcmc file
treefile<-"populations_chunks.mcmcTree.xml" ## finestructure tree file
### 2) EDIT THIS PATH TO WHERE YOU WANT THE PLOTS:
plotsFolder <- "~/Plocha/fineRADstructure"
### 3) SET VALUES FOR THESE VARIABLES: "analysisName" will be included in output plots
analysisName <- "Erinaceus"; maxIndv <- 100000; maxPop<-100000
...
pdf(file=paste(analysisName,"-SimpleCoancestry.pdf",sep=""),height=25,width=25)
...
pdf(file=paste(analysisName,"-PopAveragedCoancestry.pdf",sep=""),height=20,width=20)
plotFinestructure(tmpmat,dimnames(tmpmat)[[1]],dend=tdend,cols=some.colorsEnd,cex.axis=1.1,edgePar=list(p.lwd=0,t.srt=90,t.off=-0.1,t.cex=1.2))
dev.off()
...
pdf(file=paste(analysisName,"-PopAveragedCoancestry2.pdf",sep=""),height=25,width=25)
